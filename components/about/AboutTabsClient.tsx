"use client";

import dynamic from "next/dynamic";
import { useState } from "react";

import { aboutTabs, type AboutTab } from "@/data/about";

const panelLoaders = {
  "Visi & Misi": () => import("./panels/VisiMisiPanel"),
  Kurikulum: () => import("./panels/KurikulumPanel"),
  Fitur: () => import("./panels/FiturPanel"),
  Tim: () => import("./panels/TimPanel"),
  Kontak: () => import("./panels/KontakPanel"),
} as const;

const VisiMisiPanel = dynamic(panelLoaders["Visi & Misi"]);
const KurikulumPanel = dynamic(panelLoaders.Kurikulum);
const FiturPanel = dynamic(panelLoaders.Fitur);
const TimPanel = dynamic(panelLoaders.Tim);
const KontakPanel = dynamic(panelLoaders.Kontak);

type AboutTabsClientProps = {
  defaultTab?: AboutTab;
};

const panelMap = {
  "Visi & Misi": VisiMisiPanel,
  Kurikulum: KurikulumPanel,
  Fitur: FiturPanel,
  Tim: TimPanel,
  Kontak: KontakPanel,
} as const;

export default function AboutTabsClient({ defaultTab = "Kurikulum" }: AboutTabsClientProps) {
  const [activeTab, setActiveTab] = useState<AboutTab>(defaultTab);
  const ActivePanel = panelMap[activeTab];

  const prefetchTabPanel = (tab: AboutTab) => {
    void panelLoaders[tab]();
  };

  return (
    <>
      <div className="no-scrollbar mt-10 flex w-full items-center justify-start gap-4 overflow-x-auto border-b border-stone-300 pb-5 sm:justify-between sm:gap-3">
        {aboutTabs.map((tab) => {
          const isActive = tab === activeTab;

          return (
            <button
              key={tab}
              type="button"
              onClick={() => setActiveTab(tab)}
              onMouseEnter={() => prefetchTabPanel(tab)}
              onFocus={() => prefetchTabPanel(tab)}
              className={
                isActive
                  ? "shrink-0 whitespace-nowrap rounded-md bg-black px-6 sm:px-16 py-2 sm:py-3 text-base sm:text-lg font-semibold text-white shadow-[4px_4px_0_#d14a35] sm:shadow-[6px_6px_0_#d14a35] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[5px_5px_0_#d14a35] sm:hover:shadow-[7px_7px_0_#d14a35] active:translate-y-0.5 active:shadow-[2px_2px_0_#d14a35]"
                  : "shrink-0 whitespace-nowrap rounded-md px-6 sm:px-16 py-2 sm:py-3 text-base sm:text-lg font-semibold text-stone-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-stone-100 hover:text-stone-900 active:translate-y-0.5"
              }
            >
              {tab}
            </button>
          );
        })}
      </div>

      <ActivePanel />
    </>
  );
}
